...
class ApiController extends CoreController
{
        ...
        if (!empty($leads)) {
            ...
            foreach ($leads as $lead) {
                ...
                if (!empty($data->fonction)) {
                    $data->fonction = $this->getDoctrine()->getRepository(
                        'TellawLeadsFactoryBundle:ReferenceListElement'
                    )->getNameUsingListCode($functionListCode, $data->fonction);
                }

                //libellé ville
                if (!empty($data->ville_id)) {
                    $data->ville = $this->getDoctrine()->getRepository(
                        'TellawLeadsFactoryBundle:ReferenceListElement'
                    )->getNameUsingListCode('ville', $data->ville_id);
                }
    ...
    public function addToSandbox(Request $request)
    {
        ...
        // Vérifie si la leads existe dans Elasticsearch via le Uniq Id avant de la créer
        if ($this->checkIfLeadExistsByUniqIdInES($uniqId)) {
            return new Response("A lead with this uniqId is already in the system.");
        }
     ...
     * @return bool
     */
    private function checkIfLeadExistsByUniqIdInES($uniqId)
    {

        $esUrl = $this->get("preferences_utils")->getUserPreferenceByKey('SEARCH_URL_AND_PORT_ELASTICSEARCH');
        $queryUrl = $esUrl.'_search?q=content.uniqId:'.$uniqId;
    ...
    public function updateInSandbox(Request $request)
    {
        ...
        $leadsSandbox = $this->getDoctrine()->getRepository('TellawLeadsFactoryBundle:LeadsSandbox')->findOneByUniqId(
            $uniqId
        );
        // Si la DI n'est Plus ou Pas dans la Sandbox
        // On va tenter de la créer
        if ($leadsSandbox == null) {
            return $this->addToSandbox($request);
    ...
    public function postLeadAction(Request $request)
    {
        ...
        try {
            $form = $this->getDoctrine()->getRepository('TellawLeadsFactoryBundle:Form')->findOneByCode(
                $data['formCode']
            );
            ...
            if (array_key_exists(
                    'enableApiConfirmationEmail',
                    $config["configuration"]
                ) && $config["configuration"]["enableApiConfirmationEmail"] == true
            ) {
